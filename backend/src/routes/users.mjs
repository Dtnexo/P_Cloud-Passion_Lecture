  ...
  User.findByPk(userId).then((user) => {
    ...
    Ouvrage.findAll({ where: { user_fk: userId } }).then((ouvrages) => {
      if (ouvrages.length === 0) {
        res.status(200).json({ error: "Aucun ouvrages n'a été trouvé" });
      } else {
...
userRouter.get("/:id/evaluations", (req, res) => {
  const userId = req.params.id;
  User.findByPk(userId).then((user) => {
    if (!user) {
      return res.status(404).json({ error: "Utilisateur non trouvé" });
    }
    Evaluer.findAll({ where: { user_fk: userId } })
      .then((evaluations) => {
        if (evaluations.length === 0) {
          return res
            .status(200)
            .json({ error: "Aucune évaluation n'a été trouvée" });
        }
        res.status(200).json({
          message: "La liste des évaluations a été trouvée",
          data: evaluations,
        });
      })
      .catch((_) => {
        res.status(500).json({
          message:
            "Erreur lors de la récupération des évaluations! Veuillez réessayer dans quelques instants!",
        });
      });
  });
});
